...
import com.gmail.ecommerce.dto.perfume.PerfumeRequest;
import com.gmail.ecommerce.dto.perfume.FullPerfumeResponse;
import com.gmail.ecommerce.dto.user.BaseUserResponse;
import com.gmail.ecommerce.dto.user.UserResponse;
import com.gmail.ecommerce.mapper.OrderMapper;
import com.gmail.ecommerce.mapper.PerfumeMapper;
import com.gmail.ecommerce.mapper.UserMapper;
...
public class AdminController {
    ...
    public ResponseEntity<FullPerfumeResponse> addPerfume(
        ...
        return ResponseEntity.ok(
                perfumeMapper.savePerfume(perfume, file, bindingResult)
        );
    }
    ...
    public ResponseEntity<List<OrderResponse>> getUserOrdersByEmail(
        ...
        HeaderResponse<OrderResponse> response =
                orderMapper.getUserOrders(userEmail, pageable);
    ...
    public ResponseEntity<List<BaseUserResponse>> getAllUsers(
        ...
        HeaderResponse<BaseUserResponse> response =
                userMapper.getAllUsers(pageable);
    ...
    public ResponseEntity<ExecutionResult> getUserByQuery(
            @RequestBody GraphQLRequest request
    ) {
        return ResponseEntity.ok(
                graphQLProvider.getGraphQL().execute(request.getQuery())
        );
    }

    @PostMapping(GRAPHQL_USER_ALL)
    public ResponseEntity<ExecutionResult> getAllUsersByQuery(
            @RequestBody GraphQLRequest request
    ) {
        return ResponseEntity.ok(
                graphQLProvider.getGraphQL().execute(request.getQuery())
    ...
    public ResponseEntity<ExecutionResult> getAllOrdersQuery(
            @RequestBody GraphQLRequest request
    ) {
        return ResponseEntity.ok(
                graphQLProvider.getGraphQL().execute(request.getQuery())
