...
import {
    ...
    HeaderResponse,
    PerfumeResponse,
    PerfumesSearchRequest
} from "../../types/types";

import {
    gePerfumesByIdsQuery,
    getAllPerfumesByQuery
} from "../../utils/graphql-query/perfume-query";
...
export const fetchPerfumes = createAsyncThunk<HeaderResponse<PerfumeResponse>, number>(
    ...
    async (page) => {
        ...
        return {
            items: response.data,
            pagesCount: Number(response.headers["page-total-count"] || 0),
            totalElements: Number(response.headers["page-total-elements"] || 0)
        };
    }
);
...
export const fetchPerfumesByFilterParams = createAsyncThunk<HeaderResponse<PerfumeResponse>, FilterParamsType>(
    ...
    async (filter) => {
        ...
        const response = await RequestService.post(
            `${PERFUMES_SEARCH}?page=${filter.currentPage}`,
            filter
        );
...
export const fetchPerfumesByInputText = createAsyncThunk<HeaderResponse<PerfumeResponse>, PerfumesSearchRequest>(
    ...
    async (data) => {
        ...
        const response = await RequestService.post(
            `${PERFUMES_SEARCH_TEXT}?page=${data.currentPage}`,
            data
        );
